...
namespace OP_Macro
{
    public partial class TextAdvancedSettings : Form
    {
        ...
        private IntPtr HookCallbackThird(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN)
            {
                int vkCode = Marshal.ReadInt32(lParam);
                newThirdKey = (uint)vkCode;
                displayBoxThird.Text = ((Keys)vkCode).ToString();
                StopKeyHook();
            }
            return CallNextHookEx(_hookID, nCode, wParam, lParam);
        }
        ...
        private void startButtonThird_Click(object sender, EventArgs e)
        {
            if (!hotkeyDetecting)
            {
                _proc = HookCallbackThird;
                _hookID = SetWindowsHookEx(WH_KEYBOARD_LL, _proc, GetModuleHandle(null), 0);
                hotkeyDetecting = true;
                displayBoxThird.Text = "Натисни бутон...";
            }
        }
